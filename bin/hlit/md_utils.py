...
from template import INSTRUCTOR_TEMPLATE, STUDENT_TEMPLATE
...
def parse_md_header(header):
    header_info = dict(
        x.replace(" ", "").split(":") for x in header.split("\n") if x != ""
    )
...
def replace_md_headings_with_tsx(content, _):
    ...
    for match in matches:
        ...
        content = content.replace(
            match, f"<H{h_num}{underline}>{match_text}</H{h_num}>"
        )
...
def convert_to_tsx_template_instructor(
    student_content, instructor_content, name_prefix
):
...
def convert_to_tsx_template(student_file_name, instructor_file_name, name_prefix):
    ...
    instructor_content = convert_to_tsx_template_instructor(
        student_content, instructor_content, name_prefix
    )
...
def styled_link(content, prefix):
    content = content.replace(
        'import { StyledLink } from "~/content/styledComponents.tsx"\n', ""
    )
    ...
    content = add_import_to_file(
        content, 'import { ButtonWebsite } from "~/components/Buttons";'
    )
...
def replace_md_lists_with_ul(content, prefix):
    ...
    content = add_import_to_file(
        content, 'import UnorderedList from "~/components/List/Unordered";'
    )
